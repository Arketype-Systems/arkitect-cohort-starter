...
export interface Athlete { id: string; firstName: string; lastName: string; sex: AthleteSex; dateOfBirth: string; grade: string; sport: string; position: string; sports: string[]; positions: string[]; group: string; photoDataUrl?: string; createdAt: string; synthetic?: boolean }
...
export interface ComparisonArchetype { id: string; name: string; active: boolean; priority: number; ageMin?: number; ageMax?: number; sexes: AthleteSex[]; sports: string[]; positions: string[]; grades: string[]; levels: string[]; createdAt: string; synthetic?: boolean }
export interface MetricScore { metric: MetricStandard; value: number; points: number; band: ScoreBand; percentile?: number; cohortSize?: number }
export interface AthleteReport { athlete: Athlete; version: StandardsVersion; profile: StandardsProfile; archetype?: ComparisonArchetype; sessions: AssessmentSession[]; latestSession?: AssessmentSession; scores: MetricScore[]; overall: number | null; maxPoints: number; complete: boolean; missing: string[]; strengths: MetricScore[]; priorities: MetricScore[]; percentileAverage: number | null }
export interface BrandSettings { organizationName: string; productName: string; logoDataUrl?: string; primaryColor: string; accentColor: string; inkColor: string; bodyFont: string; displayFont: string; direction: 'technical' | 'classic' | 'bold' }
