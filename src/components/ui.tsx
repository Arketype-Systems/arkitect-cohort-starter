...
export function ScoreBadge({ score, maxPoints = 20, label = 'Point total' }: { score: number | null; maxPoints?: number; label?: string }) { const ratio = score === null ? 0 : score / Math.max(maxPoints, 1); return <div className={`score-badge ${score === null ? 'neutral' : ratio >= .75 ? 'good' : ratio >= .5 ? 'ready' : 'attention'}`}><strong>{score ?? 'Incomplete'}</strong><span>{score === null ? 'Required metrics missing' : `${label} · ${score} of ${maxPoints}`}</span></div> }
