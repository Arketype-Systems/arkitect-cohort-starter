...
export default function AthleteReportPage() {
  ...
  const { athletes, sessions, measurements, standards, archetypes } = useData()
  ...
  return <div className="page report-page">
    ...
    <article className="report-document">
      <section className="report-sheet report-cover">
        <ReportHeader brand={brand} title={identity} subtitle={`${athlete.sports.join(' / ') || 'Sport not set'} · ${athlete.positions.join(' / ') || 'Position not set'} · Grade ${athlete.grade || 'not set'} · ${athlete.group}`} date={report.latestSession?.date} version={`${version.name} · v${version.version}`} profile={report.archetype?.name ?? 'No matching comparison group'} />
        ...
        <div className="report-cover-grid">
          ...
          <div className="report-radar-panel"><div className="section-heading"><div><p className="eyebrow">Percentile shape</p><h2>{report.archetype?.name ?? 'No matching comparison group'}</h2></div>{report.percentileAverage !== null && <strong>{report.percentileAverage}th avg.</strong>}</div><RadarChart scores={report.scores} label={report.archetype?.name ?? 'the selected comparison group'} /><p className="report-method-note">Percentiles compare published results from athletes matching this database archetype. Standards profiles still control points and remain separate.</p></div>
        </div>
        <div className="report-summary-strip"><div><span>Tests reported</span><strong>{report.scores.length}</strong></div><div><span>Comparison archetype</span><strong>{report.archetype?.name ?? 'Not matched'}</strong></div><div><span>Scoring profile</span><strong>{report.profile.name}</strong></div><div><span>Standards version</span><strong>v{version.version}</strong></div></div>
        <ReportFooter athleteId={athlete.id} version={version.version} page="1 / 3" />
      </section>

      <section className="report-sheet">
        <ReportHeader brand={brand} title="Testing detail" subtitle={identity} date={report.latestSession?.date} version={`v${version.version}`} profile={report.archetype?.name ?? 'No matching comparison group'} compact />
      ...
      <section className="report-sheet">
        <ReportHeader brand={brand} title="Coaching interpretation" subtitle={identity} date={report.latestSession?.date} version={`v${version.version}`} profile={report.archetype?.name ?? 'No matching comparison group'} compact />
        ...
        <section className="report-contract-grid"><div><span>Scoring contract</span><strong>Deterministic 0 to 4 grades</strong><p>Lower and higher direction are evaluated by the versioned bands. Missing required tests never produce a total.</p></div><div><span>Comparison contract</span><strong>Matched archetype and exact version</strong><p>Current database archetype rules select percentile peers. The exact standards version keeps compared measurements compatible.</p></div><div><span>Data boundary</span><strong>Generated on this device</strong><p>No athlete record is sent to Arkitect, Arketype, Supabase, or another hosted service by this starter.</p></div></section>
