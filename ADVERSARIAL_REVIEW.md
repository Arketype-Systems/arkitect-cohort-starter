...
| Cross-cohort percentile mixing | Pass | Percentile peers must match the same persisted comparison archetype on each assessment date and share the exact standards version. Standards profiles never define the percentile cohort. Unit fixtures cover age, sex, sport, priority, and cohort isolation. |
...
| Ranking distortion | Pass | Point rankings include complete published totals only. Raw metric ranking respects scoring direction. Rankings consumes comparison archetypes as filters but does not define or own them. Filters also cover sex, assessment-date age group, sport, position, and grade. |
...
| One-route fakery | Pass | Route/component fixtures and deep-link browser coverage exercise the application routes, browser history, and reload behavior. Archetype management is a persisted Database workflow, while Rankings remains a dedicated leaderboard route. |
...
- Unit, route, database, and importer tests: 51 passed
