...
6. Every score and report identifies its standards version. If a metric band, unit, weight, direction, or required flag changes, create a new version and update explicit fixtures.
7. Keep CSV parsing on Papa Parse and persistence on Dexie. Show row errors without discarding unrelated valid rows and prevent duplicate athlete creation.
8. Preserve real routes, deep links, browser history, reload behavior, and responsive layouts at 390, 768, 1024, and 1440 pixels.
9. Use complete, calm coach facing copy. Do not add visible AI labels. Do not use dashes as prose connectors.
